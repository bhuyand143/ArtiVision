...
const CreatePost = () => {
  ...
  const generateImage=async()=>{
    if(form.prompt && form.name){
      try{
        ...
        const response= await fetch('https://artivision.onrender.com/api/v1/dalle',{
          method:'POST',
          headers:{
  ...
  const handleSubmit = async(e) => {
    ...
    if(true){
      setLoading(true);
      try{
        const response= await fetch('https://artivision.onrender.com/api/v1/post',{
          method:'POST',
          headers:{
            'Content-Type':'application/json'
          },
      ...
      alert('Please enter a prompt to Generate a Photo!');
    }
  };
